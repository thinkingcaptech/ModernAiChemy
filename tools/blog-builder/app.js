...
// Get best available API key from unified storage
function getApiKeyWithProvider() {
  try {
    const stored = localStorage.getItem('api_keys');
    if (stored) {
      const keys = JSON.parse(stored);
      // Return first available key with its provider
...
function getGeminiApiKey() {
  const result = getApiKeyWithProvider();
  return result?.key || null;
}

// Call AI API directly (BYOK) - supports all providers
async function callAiAPI(prompt) {
  const apiData = getApiKeyWithProvider();
  
  if (!apiData) {
    throw new Error('No AI API key configured. Please add your key in Settings.');
  }
  
  const { key, provider } = apiData;
  const config = AI_PROVIDERS[provider];
  
  console.log(`[BlogBuilder] Using ${config.name}...`);
...
// Generate a single localized post
async function generateLocalizedPost(baseArticle, city, tone, length) {
  const prompt = `You are an expert SEO content writer. Take this base article template and create a unique, localized version for ${city}.

Base Template:
${baseArticle}

Instructions:
- Replace all [CITY] placeholders with "${city}"
- Make the content feel locally relevant to ${city}
- Tone: ${tone}
- Length: ${length} (short=300 words, medium=500 words, long=800 words)
- Keep SEO-friendly structure with headers
- Make it unique, not just find-replace
...
async function handleGenerate() {
  ...
  const apiData = getApiKeyWithProvider();
  if (!apiData) {
    if (confirm('No AI API key configured.\n\nWould you like to add your API key in Settings?')) {
      window.location.href = '/settings.html';
    }
    return;
  }

  const providerName = AI_PROVIDERS[apiData.provider]?.name || 'AI';
  console.log(`[BlogBuilder] Ready to generate with ${providerName}`);
  ...
  const estimatedInputTokens = baseArticle.length * 0.25 * cities.length;
  const estimatedOutputTokens = 2000 * cities.length;
  ...
  resultsContainer.innerHTML = '<p class="text-gray-400 text-xs">Processing...</p>';

  try {
    // Generate posts client-side using BYOK
    const posts = [];
    for (let i = 0; i < cities.length; i++) {
      const city = cities[i];
      statusEl.textContent = `Generating post ${i + 1} of ${cities.length}: ${city}...`;
      
      try {
        const content = await generateLocalizedPost(baseArticle, city, toneEl.value, lengthEl.value);
        posts.push({
          city,
          content,
        ...
        console.error(`Failed to generate for ${city}:`, err);
        posts.push({
          city,
          content: `Error generating content for ${city}: ${err.message}`,
          slug: city.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
          error: true
        });
      }
    ...
    statusEl.textContent = `✓ Generated ${successCount} of ${cities.length} localized posts.`;
...
function renderResults(posts) {
  ...
  if (!posts.length) {
    resultsContainer.innerHTML = "<p class='text-xs text-gray-400'>No posts generated yet.</p>";
    return;
  }
  ...
  posts.forEach((post, idx) => {
    ...
    // Trigger the alchemical celebration animation with staggered delay
    setTimeout(() => {
      card.classList.add('post-card--celebrate');
      // Remove the class after animation completes so it can be re-triggered
      setTimeout(() => {
        card.classList.remove('post-card--celebrate');
      }, 5000);
    }, idx * 150); // Stagger each card's animation by 150ms
  });
}
